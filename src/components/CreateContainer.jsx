  ...
  const saveDetails = () => {
    ...
    try {
      if (!title || !calories || !imageAsset || !category) {
        setFields(true);
        setMsg("Required Fields can't be filled");
        setAlertStatus("danger");
        setTimeout(() => {
          setFields(false);
          setIsLoading(false);
  ...
  return (
    <div className="w-full min-h-screen h-auto flex items-center justify-center">
      <div className="w-[90%] md:w-[75%] border border-gray-300 rounded-lg p-4 flex flex-col items-center justify-center gap-4">
        ...
        <div className="w-full " onChange={(e) => setCategroy(e.target.value)}>
          <select className="w-full outline-none text-base border-b-2 border-gray-100 p-2 rounded-md cursor-pointer">
            ...
            {categories &&
              categories.map((val) => {
                return (
                  <option
                    className="text-base border-0 outline-none capatalize bg-white text-headingColor"
                    key={val.id}
                    value={val.urlParamName}
                  >
                    {val.name}
                  </option>
                );
